...
The export script reads credentials exclusively from environment variables — there are no hardcoded fallbacks. For GitHub Actions to connect to the database, the following secrets must be set on the repository.
...
| Secret | Description |
|--------|-------------|
| `DB_HOST` | PostgreSQL host (Hetzner VPS IP) |
...
Secrets are encrypted, never exposed in logs, and only injected into Actions runs — they are not accessible to forks or pull requests from external contributors.
...
# Export fresh data locally
# Credentials are read from environment variables — create a .env file:
#   DB_HOST=...
#   DB_PORT=5432
#   DB_USER=...
#   DB_PASSWORD=...
# (.env is gitignored and never committed)
source .env && python scripts/export_redlines_data.py

# Build and deploy app changes to GitHub Pages (only needed when React code changes)
npm run build
npx gh-pages -d dist

# Trigger data update manually via GitHub Actions (no rebuild needed)
